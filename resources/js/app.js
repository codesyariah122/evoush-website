/**
 * First we will load all of this project's JavaScript dependencies which
 * includes Vue and other libraries. It is a great starting point when
 * building robust, powerful web applications using Vue and Laravel.
 */

require('./bootstrap');

// components module and assets
import Vue from 'vue'
import VueAxios from 'vue-axios'
import BootstrapVue from 'bootstrap-vue'
import axios from 'axios'
import VueSweetalert2 from 'vue-sweetalert2'
import "bootstrap-vue/dist/bootstrap-vue.css";
import 'bootstrap/dist/css/bootstrap.css'
import 'jquery/dist/jquery.min'
import 'popper.js/dist/popper.min'
import 'bootstrap/dist/js/bootstrap.min'

// components web
import Home from './pages/Home/index'
import Popup from './components/molecules/popup'
import About from './pages/About/index'
import Product from './pages/Product/index'
import Member from './pages/Member/index'
import MarketingPlan from './pages/MarketingPlan/index'
import ProfileLogin from './pages/ProfileLogin/index'
import ProfilePublic from './pages/ProfilePublic/index'


Vue.config.productionTip = false
Vue.use(VueAxios, axios)
Vue.use(BootstrapVue)
Vue.use(VueSweetalert2)
Vue.component('pagination', require('laravel-vue-pagination'))
// web component vue template
new Vue({
	el: '#home',
	components: {Home},
	render: h => h(Home)
})

new Vue({
	el: '#popup',
	components: {Popup},
	render: h => h(Popup)
})

new Vue({
	el: '#about',
	components: {About},
	render: h => h(About)
})

new Vue({
	el: '#product',
	components: {Product},
	render: h => h(Product)
})

new Vue({
	el: '#member',
	components: {Member},
	render: h => h(Member)
})

new Vue({
	el: '#profile-login',
	components: {ProfileLogin},
	render: h => h(ProfileLogin)
})

new Vue({
	el: '#profile-public',
	components: {ProfilePublic},
	render: h => h(ProfilePublic)
})


new Vue({
	el: '#marketing-plan',
	components: {MarketingPlan},
	render: h => h(MarketingPlan)
})



// Component assets for webpage evoush for blade template
new Vue({
	el: '#login',
	data(){
		return {
			showing: false,
			show: `<i class="fas fa-fw fa-eye"></i>`,
			hide: `<i class="fas fa-low-vision"></i>`
		}
	},

	methods: {
		historyBack(){
			window.history.back()
		},

		showPassword(){
			const password = document.querySelector('#password')
			if(password.type === "password"){
				password.type = "text"
				this.showing = true
			}else{
				password.type = "password"
				this.showing = false
			}			
		}
	}
})


new Vue({
	el: '#nav',

	mounted(){
		this.scrollNav()
	},

	methods: {
		scrollNav(){
			window.addEventListener('scroll', function(){
				let scrollPos = window.scrollY
				let nav = document.querySelector('.navbar-evoush')
				let isDesktop = window.matchMedia('only screen and (min-width: 992px)').matches;
	          // console.log(scrollPos)
		          if(!isDesktop){
		          	nav.classList.add('mobileNav')
		          	if(scrollPos > 150){
		          		nav.classList.remove('navbar-light')
		          		nav.classList.add('navbar-light')
		          		nav.classList.add('transition')
		          		if(scrollPos > 400){
		          			nav.classList.add('bg-white')
		          		}else{
		          			nav.classList.remove('bg-white')
		          			nav.classList.add('transparent-nav')
		          		}
		          	}else{
		          		nav.classList.remove('transparent-nav')
		          		nav.classList.remove('bg-white')
		          		nav.classList.remove('transition')
		          	}
		          }else{
		          	nav.classList.remove('mobileNav')
		          	if(scrollPos > 150){
		          		nav.classList.remove('transparent-nav')
		          		nav.classList.add('transition')
		          		if(scrollPos > 400){
		          			nav.classList.add('bg-dark')
		          		}else{
		          			nav.classList.remove('bg-dark')
		          			nav.classList.add('transparent-nav')
		          		}
		          	}else{
		          		nav.classList.remove('transparent-nav')
		          		nav.classList.remove('bg-dark')
		          		nav.classList.remove('transition')
		          	}
		          }
	      	})
		},

		searchInput(e){
			console.log(e.target.value)
		}
	}
})


// footer web
new Vue({
	el: '#template-brand-footer',
	data(){
		return {
			comp: "PT. Pineleng Indah Cemerlang",
			cops: "Hak Cipta Terpelihara Oleh",
			logoBrand: "voush",
			brand: "Evoush",
			country: "Indonesia",
			year: "",
			slogan: `<span style="font-family: 'Walkway'; color: white;">Your Eternal Future </span>`
		}
	}
})


new Vue({
	el:'#product-footer',
	data(){
		return {
			links: [

			{id:1, name:"Healthy Food", link: "/evoush/application/product/healthy"},
			{id:2, name:"Skincare Beauty", link: "/evoush/application/product/beauty"},
			{id:3, name:"Support System", link: "/evoush/application/salaam"},
			{id:4, name:"Cargo Shipping", link: "/evoush/application/cargo"},
			{id:5, name:"Contact", link: "/contact"}
			]
		}
	}
})

new Vue({
	el: '#site-link-footer',
	data(){
		return {
			links: [
			{id:1, name:"About", url: "/about"},
			{id:2, name:"Management", url: "/management"},
			{id:3, name:"Top Leaders", url: "/top-leaders"},
			{id:4, name:"Success Stories", url: "/success-stories"},
			{id:5, name:"Event/Tickets", url: "/event"},
			{id:6, name:"News", url: "/articles"},
			{id:7, name:"Life Style", url: "#"},
			{id:8, name:"Marketing Plan", url: "/marketing-plan"}
			]
		}
	}
})

new Vue({
	el: '#social-link-footer',
	data(){
		return {
			email: {
				href: 'mailto:evoushofficiall@gmail.com',
				label: 'evoushofficiall@gmail.com',
				icon: `<i class="fas fa-envelope-open-text fa-fw text-white"></i>`
			},
			wa : {
				href: 'https://wa.me/6282114610011?text=Hallo%20Admin%20Evoush%20Official',
				label: 'Evoush Admin / Whatsapp',
				icon: `<i class="fas fa-mobile-alt fa-fw text-white"></i>`
			},
			contact : {
				href: '/contact',
				label: 'Kontak Kami',
				icon: `<i class="fas fa-envelope fa-fw text-white"></i>`
			},
			socials: [

			{
				name: 'youtube',
				link: 'https://www.youtube.com/channel/UCIzNgeNDD58z8XNppkopwzw',
				icon: 'data:image/png;base64,'
			},

			{
				name: 'facebook',
				link: 'https://www.facebook.com/evoush.evoush.12',
				icon: 'data:image/png;base64,'
			},

			{
				name: 'twitter',
				link: 'https://twitter.com/EvoushOfficial',
				icon: 'data:image/png;base64,'
			},

			{
				name: 'instagram',
				link: 'https://www.instagram.com/evoush.official',
				icon: 'data:image/png;base64,'
			}

			]
		}
	}
})

new Vue({
	el: '#evoush-app-footer',
	data(){
		return {
			links: [
			{id:1, name:"Contat", link: "/contact"},
			{id:2, name:"Quran Digital", link: "/evoush/app/quran"},
			{id:3, name:"Cargo Shipping", link: "/evoush/app/cargo"}
			]
		}
	}
})

new Vue({
	el: '#footer-copyright',
	data(){
		return {
			comp: "PT. Pineleng Indah Cemerlang",
			cops: "Hak Cipta Terpelihara Oleh",
			logoBrand: "voush",
			brand: "Evoush",
			country: "Indonesia",
			year: ""
		}
	},

	mounted(){
		this.getYear()
	},

	methods: {
		getYear(){
			const year = new Date()
			this.year = year.getFullYear()
			// console.log(this.year)
		}
	}

})
// end footer web